			...
			this.hvsc = $.get("php/music.php", {
				...
				this.validateData(data, function(data) {
					...
					if (this.path == "" && !this.isSearching) {
						...
						// Number of search results and page counter
						pathText = '<div class="results unselectable">'+data.results+' result'+(data.results == 1 ? '': 's')+' '+
	...
	buildTags: function(tags, types, ids) {
		...
		$.each(tags, (i, tag) => {
			...
			/*} else if (tag == "Compo") {*/
				...
				// If you want to change the order of tags, see getTagsAndTypes() in 'functions.tags.php'
