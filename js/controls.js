...
Controls.prototype = {
	...
	onMouseUp: function(event, autoProgress) {
		...
		if (id.substr(0, 7) == "subtune") {
			...
			if (isAutoProgress && main.getUserToggle("skip-short"))  {
		...
		if (id.substr(0, 4) == "skip") {
			...
			var subtune = main.getUserToggle("first-subtune") ? 0 : browser.songs[browser.songPos].startsubtune;
			// The default is too short, but what about the subsequent sub tunes in it?
			if (isAutoProgress && main.getUserToggle("skip-short") && songLength < 10) {
			...
			SID.load(subtune, browser.getLength(subtune), browser.songs[browser.songPos].fullname, function(error) {
				...
				if (!isAutoProgress || main.getUserToggle("mark-tune")) {
