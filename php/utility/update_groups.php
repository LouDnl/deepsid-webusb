...
try {
	...
	foreach($groups as $member) {
		$new = $db->prepare('SELECT files FROM folders WHERE collection_path = :collection_path LIMIT 1');
		...
		$db->query('UPDATE folders SET files = '.$count.' WHERE collection_path = "'.$target.'" LIMIT 1');
	...
	foreach($unique_groups as $group) {
		...
		$db->query('UPDATE folders SET files = '.$group->member_count.' WHERE collection_path = "'.$collection_path.'" LIMIT 1');
