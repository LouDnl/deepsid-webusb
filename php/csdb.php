...
require_once("lib/functions.gb64.php");
...
if ($csdb_type == 'sid') {
	...
	if (isset($csdb->SID->UsedIn)) {
		...
		foreach($releases as $release) {
			...
			*/
			...
			$primary_title_icon = $primary_id == (int)$release->ID ? '<div class="primary-title-icon"></div>' : '';
			...
			$entry =
				'<tr>'.
					...
					'</td>'.
					'<td class="info">'.
						'<a class="'.($can_show_internally ? 'internal ' : '').'name" href="http://csdb.chordian.net/?type=release&id='.$release->ID.'" data-id="'.$release->ID.'" target="_blank">'.$adapted_name.'</a>'.$primary_title_icon.'<br />'.
	...
	$sticky = '<h2 class="ellipsis csdb-ellipsis" title="'.$csdb->SID->Name.'">'.$csdb->SID->Name.'</h2>'.
		'<a class="clipboard" href="//deepsid.chordian.net?tab=csdb&csdbtype=sid&csdbid='.$csdb->SID->ID.'" title="Permalink">'.$svg_permalink.'</a>'.
	...
	// At one point there were extra controls visible for administrators only here, but they were removed.
	// Adding this on the page is not a good idea because of the cache system. Add controls for the page in
	// another location, such as e.g. the context menu.
